...
class KCAPTCHA{
	...
	function KCAPTCHA(){
		...
		$this->img2=imagecreatetruecolor($width, $height+($show_credits?12:0));
		$foreground=imagecolorallocate($this->img2, $foreground_color[0], $foreground_color[1], $foreground_color[2]);
		$background=imagecolorallocate($this->img2, $background_color[0], $background_color[1], $background_color[2]);
		imagefilledrectangle($this->img2, 0, 0, $width-1, $height-1, $background);		
		imagefilledrectangle($this->img2, 0, $height, $width-1, $height+12, $foreground);
		$credits=empty($credits)?$_SERVER['HTTP_HOST']:$credits;
		imagestring($this->img2, 2, $width/2-imagefontwidth(2)*strlen($credits)/2, $height-2, $credits, $background);
